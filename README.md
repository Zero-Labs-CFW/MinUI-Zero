...
* **What it syncs**: Saves (on by default: save files, save states and their thumbnails, Favorites, Collections), Games and Game Configs (off by default). Each device chooses what it takes: a category copies to a device only when that device has it on, so Games on one side brings games to that side only. Recently Played stays per device, and device settings never sync.
